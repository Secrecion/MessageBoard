...
suite('Functional Tests', function() {
    suite("10 functional tests", function(){
        ...
        test("Deleting a thread with the incorrect password: DELETE request to /api/threads/{board} with an invalid delete_password", function(done){
            ...
            .set("content-type", "application/json")
            .send({thread_id:testThread_id, delete_password:"test1"})
            .end(function(err, res){
                assert.equal(res.status, 200);
        ...
        test("Creating a new reply: POST request to /api/replies/{board}", function(done){
            ...
            .set("content-type", "application/json")
            .send({thread_id:testThread_id, text:"test reply", delete_password:"testreply"})
            .end(function(err, res){
                assert.equal(res.status, 200);
        ...
        test("Deleting a reply with the incorrect password: DELETE request to /api/replies/{board} with an invalid delete_password", function(done){
            ...
            .set("content-type", "application/json")
            .send({thread_id:testThread_id,reply_id:testReply_id, delete_password:"testreply1"})
            .end(function(err, res){
                assert.equal(res.status, 200);
                assert.equal(res.text, "incorrect password");
                done();
        ...
        test("Deleting a reply with the correct password: DELETE request to /api/replies/{board} with a valid delete_password", function(done){
            ...
            .delete("/api/replies/test-board")
            .set("content-type", "application/json")
            .send({thread_id:testThread_id,reply_id:testReply_id, delete_password:"testreply"})
            .end(function(err, res){
                assert.equal(res.status, 200);
